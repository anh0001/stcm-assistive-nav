...
from .map_utils import _build_llm_summary, read_stcm_json
...
def _translation_matrix(x: float, y: float, z: float) -> np.ndarray:
    ...
    mat[:3, 3] = [float(x), float(y), float(z)]
    return mat
...
class RosbagRgbIndex:
    ...
    def _load_rgb_rows(self) -> list[tuple[int, int]]:
        ...
        return [(int(row["id"]), int(row["timestamp"])) for row in cursor.fetchall()]
    ...
    def _build_index(self) -> None:
        if self.storage_id != "sqlite3":
            raise RuntimeError(f"GT annotation backend currently supports sqlite3 rosbag storage only, got '{self.storage_id}'")
        topic_types = {
        ...
        for topic in required_topics + [self.camera_info_topic]:
        ...
        while aux_index < len(aux_rows) or rgb_index < len(self._rgb_rows):
            ...
            elif topic == self.camera_info_topic and self.image_width is None and getattr(msg, "width", None):
    ...
    def _resolve_robot_pose(self) -> tuple[float, float, float] | None:
        chain: list[np.ndarray] = []
        visited: set[str] = set()
        current = self.base_frame
        while current != self.world_frame:
    ...
    def get_frame_image(
        ...
        *,
        ...
        cursor = conn.execute("SELECT data FROM messages WHERE id = ?", (frame.message_id,))
        row = cursor.fetchone()
        if row is None:
            raise KeyError(f"RGB message id {frame.message_id} not found")
        msg = deserialize_message(row["data"], self._rgb_message_type)
        image = self._bridge.imgmsg_to_cv2(msg, desired_encoding="rgb8")
        image = np.asarray(image)
        image = self._resize_image(image, max_width=max_width, max_height=max_height)
        with self._cache_lock:
            self._frame_image_cache[frame_index] = image
            if len(self._frame_image_cache) > 24:
                oldest = sorted(self._frame_image_cache)[:8]
                for key in oldest:
    ...
    def map_click(
        ...
        *,
        ...
        if mode == "move":
            ...
            obj["pose"] = [float(world_x), float(world_y), float(_ensure_pose3(obj.get('pose'))[2])]
            self.current_frame_index = self.rosbag_index.nearest_frame_index(world_x, world_y)
